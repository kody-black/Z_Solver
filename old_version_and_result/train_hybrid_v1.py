# 2025年8月6日版本
# 1. 添加了领域对抗损失
# 2. 为了防止梯度爆炸：添加了梯度裁剪、warmup阶段、更换了优化器为AdamW、一致性权重逐步上升
...
from util import compute_seq_acc, Seq2SeqLoss, ConsistentLoss, ConsistentLoss_MT_Temperature, get_current_consistency_weight

# --- 参数解析器 (使用上一版验证过的稳定参数) ---
parser = argparse.ArgumentParser(description='PyTorch Captcha Training Using Stable Hybrid Model')
parser.add_argument('--dataset', default='apple', type=str, help="数据集名称")
...
parser.add_argument('--t', default=0.5, type=float, help='Mean-Teacher 温度参数')
parser.add_argument('--weight', default=1.0, type=float, help='一致性损失权重')
parser.add_argument('--lr', default=1e-3, type=float, help='学习率 (1e-3 is a good start for AdamW)')
...
if USE_CUDA:
    ...
    class_criterion, consistent_criterion = class_criterion.cuda(), consistent_criterion.cuda()
    consistent_criterion_mt, domain_criterion = domain_criterion.cuda(), domain_criterion.cuda()
...
# --- 训练历史记录 ---
history = {'train_loss_class': [], 'train_loss_consistency': [], 'train_loss_consistency_mt': [], 'train_loss_domain': [], 'train_accuracy': [], 'test_accuracy': [], 'test_accuracy_ema': []}
best_ema_acc = 0.0

# --- 打印表头 ---
TABLE_HEADER = "| Epoch | L_Class | L_Cons  | L_ConsMT| L_Domain| Train Acc | Test Acc | Test Acc EMA | Time (s) |"
...
for epoch in range(args.epoch):
    ...
    running_losses = {'class': 0.0, 'cons': 0.0, 'cons_mt': 0.0, 'domain': 0.0}
    ...
    for i, (inputs_u_w, inputs_u_s) in enumerate(target_loader):
        ...
        # --- 正常训练阶段 ---
            ...
            Lu_mt = consistency_weight * consistent_criterion_mt(logits_u_s, logits_u_w_teacher.detach())
            ...
            # 4. 总损失
            loss_all = Lx + Lu + Lu_mt + args.lambda_d * Ld

            # 记录所有损失值
            running_losses['class'] += Lx.item()
            running_losses['cons'] += Lu.item()
            running_losses['cons_mt'] += Lu_mt.item()
    ...
    history['train_loss_consistency_mt'].append(running_losses['cons_mt'] / len_dataloader)
    ...
    print(f"| {epoch:^5} | {history['train_loss_class'][-1]:<7.4f} | {history['train_loss_consistency'][-1]:<7.4f} | {history['train_loss_consistency_mt'][-1]:<7.4f} | {history['train_loss_domain'][-1]:<7.4f} | {history['train_accuracy'][-1]:<9.4f} | {history['test_accuracy'][-1]:<8.4f} | {history['test_accuracy_ema'][-1]:<12.4f} | {epoch_time:<8.2f} |")
...
ax1.plot(history['train_loss_consistency_mt'], label='L_Cons_MT (Train)')
ax1.plot(history['train_loss_domain'], label='L_Domain (Train)')
ax1.legend()
ax1.set_xlabel("Epochs")
ax1.set_ylabel("Loss")
ax1.grid(True, linestyle='--', alpha=0.6)
ax1.set_ylim(bottom=0) # 损失值不应为负
...
ax2.set_ylim(0, 1.0) # 准确率在0到1之间

# 保存结果图
path_params = f"Hybrid_{args.dataset}_{args.label.split('.')[0]}_{args.unlabeled_number}_{args.lr}_{args.seed}"
